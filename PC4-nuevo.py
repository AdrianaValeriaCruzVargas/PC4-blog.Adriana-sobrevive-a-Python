...
Hubo momentos en los que me frustraba por no recordar o no entender del todo, e incluso llegué a dudar de su utilidad en mi carrera. Pero gracias a la práctica constante y al enfoque aplicado de las tareas, logré avanzar y sentirme más segura. Fue un proceso retador, pero también muy enriquecedor.
    ...
    st.markdown("<h1 style='text-align: center;'>Mis criaturas (gráficos)</h1>", unsafe_allow_html=True)

    # Creamos una lista de gráficos
    graficos = ['Gráfico de torta de estreno de series vs. peliculas', 'Gráfico de barras de cantidad de visualizaciones según el tipo de video en tik tok', 'Mapa de mis pelis favoritas']
    ...
    if grafico_seleccionado == 'Gráfico de torta de estreno de series vs. peliculas':
        st.markdown("<div style='text-align: justify; font-size: 20px;'>El gráfico de torta representa la proporción entre películas y series estrenadas en Netflix, donde las películas ocupan el 69.6% del contenido y las series el 30.4%. Esto indica una predominancia de películas en la plataforma, lo que podría reflejar una estrategia de contenido enfocada en producciones cinematográficas.</div>", unsafe_allow_html=True)
        st.image("grafico_1_series_peliculas.png", caption='Gráfico de peliculas vs. series', width=500)
        pass
    elif grafico_seleccionado == 'Gráfico de barras de cantidad de visualizaciones según el tipo de video en tik tok':
